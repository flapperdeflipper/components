...
def _get_rest_data(hass, rest_config, arguments):
    ...
    # PATCH: HA's RestData renders params/headers templates WITHOUT any
    # variables (template.render_complex), so "{{ query }}" would raise.
    # Render them here with the tool-call arguments instead.
    for _key in (CONF_HEADERS, CONF_PARAMS):
        _values = rest_config.get(_key)
        if isinstance(_values, dict):
            rest_config[_key] = {
                k: v.async_render(arguments, parse_result=False)
...
class FunctionExecutor(ABC):
    ...
    def to_arguments(self, arguments):
        ...
        # Compatibility shim: accept "url" as an alias for the HA rest/scrape
        # schema keys. Static urls map to "resource", templated ones to
        # "resource_template" (matching homeassistant rest RESOURCE_SCHEMA).
        arguments = dict(arguments)
        if "url" in arguments and "resource" not in arguments and "resource_template" not in arguments:
            url_value = str(arguments.pop("url"))
            if "{{" in url_value:
                arguments["resource_template"] = url_value
            else:
                arguments["resource"] = url_value
        try:
...
class RestFunctionExecutor(FunctionExecutor):
    ...
    async def execute(
        ...
        # PATCH: surface the real failure instead of silently returning None
        if rest_data.last_exception is not None:
            raise HomeAssistantError(str(rest_data.last_exception)) from (
                rest_data.last_exception
            )
...
class ScrapeFunctionExecutor(FunctionExecutor):
    ...
    async def execute(
        ...
        config = function
        rest_data = _get_rest_data(hass, config, arguments)
        # PATCH: HA 2026.8 changed ScrapeCoordinator's signature - it now
        # takes (hass, config_entry, rest, rest_config, update_interval).
        # config_entry is None (we are not a config entry) and a single
        # async_refresh() is used instead of first_refresh for that reason.
